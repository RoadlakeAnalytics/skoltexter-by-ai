...
from src.config import ORIGINAL_CSV_PATH, AI_MARKDOWN_DIR, OUTPUT_HTML_FILE, WEBSITE_TEMPLATE_PATH
from .data_aggregator import read_school_csv, deduplicate_and_format_school_records, load_school_data
from .renderer import generate_final_html, write_html_output

logger = logging.getLogger(__name__)


def run_from_config(csv_path: Path | None = None, ai_markdown_dir: Path | None = None, output_file: Path | None = None) -> bool:
    csv_path = Path(csv_path) if csv_path is not None else ORIGINAL_CSV_PATH
    ai_markdown_dir = Path(ai_markdown_dir) if ai_markdown_dir is not None else AI_MARKDOWN_DIR
