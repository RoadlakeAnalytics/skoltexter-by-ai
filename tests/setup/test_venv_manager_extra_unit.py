...
def test_manage_virtual_environment_recreate_permission_error(monkeypatch, tmp_path: Path):
    ...
    class UI:
        def __init__(self):
            self.logger = SimpleNamespace(error=lambda *a, **k: captured["msgs"].append(str(a)))
    ...
    monkeypatch.setattr(vm, "safe_rmtree", lambda p: (_ for _ in ()).throw(PermissionError("denied")))
