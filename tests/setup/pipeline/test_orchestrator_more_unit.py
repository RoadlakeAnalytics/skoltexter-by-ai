...
def test_compose_and_update_with_fallback_group(monkeypatch):
    ...
    monkeypatch.setattr(orchestrator, "_TUI_UPDATER", lambda c: captured.setdefault("c", c))
    ...
    monkeypatch.setattr(ch, "Group", lambda a, b: (_ for _ in ()).throw(RuntimeError("nope")))
