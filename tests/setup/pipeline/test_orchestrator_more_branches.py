...
def test_run_pipeline_step_branches(monkeypatch):
    ...
    calls = {}
    monkeypatch.setattr(
        orch, "ui_success", lambda m: calls.setdefault("succ", m), raising=False
    )
    ...
    calls = {}
    monkeypatch.setattr(
        orch, "ui_warning", lambda m: calls.setdefault("warn", m), raising=False
    )
    ...
    calls = {}
    monkeypatch.setattr(
        orch, "ui_info", lambda m: calls.setdefault("info", m), raising=False
    )
    ok3 = orch._run_pipeline_step(
        "prompt", "program_1", Path("p.py"), "fail", "ok", skip_message="skip_msg"
    )
    assert ok3 is True
    ...
    calls = {}
    monkeypatch.setattr(
        orch, "ui_warning", lambda m: calls.setdefault("warn2", m), raising=False
    )
