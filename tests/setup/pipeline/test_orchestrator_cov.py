...
def test_run_ai_connectivity_check_interactive(monkeypatch):
    ...
    # Failure
    monkeypatch.setattr(orch, 'run_ai_connectivity_check_silent', lambda: (False, 'ERR'))
...
def test_run_pipeline_step_branches(monkeypatch):
    ...
    monkeypatch.setattr(orch, 'run_program', lambda name, path, stream_output=False: True)
    calls = {}
    monkeypatch.setattr(orch, 'ui_success', lambda msg: calls.setdefault('success', msg))
    ok = orch._run_pipeline_step('run_program_1_prompt', 'program_1', Path := types.SimpleNamespace(), 'fail', 'ok')
    assert ok is True
    # y + fail
    ...
    ok2 = orch._run_pipeline_step('run_program_1_prompt', 'program_1', Path, 'fail', 'ok')
    assert ok2 is False
    # skip
    ...
    ok3 = orch._run_pipeline_step('run_program_1_prompt', 'program_1', Path, 'fail', 'ok', skip_message='program_2_skipped')
    assert ok3 is True
    # invalid
    monkeypatch.setattr(orch, 'ask_text', lambda prompt, default='y': 'x')
    calls.clear()
    ok4 = orch._run_pipeline_step('run_program_1_prompt', 'program_1', Path, 'fail', 'ok')
    assert ok4 is False
