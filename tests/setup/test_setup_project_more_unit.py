...
setattr(
    sp, "_run_processing_pipeline_rich", _app_pipeline._run_processing_pipeline_rich
)
setattr(
    sp, "_run_processing_pipeline_plain", _app_pipeline._run_processing_pipeline_plain
)
...
def test_view_program_descriptions_plain(monkeypatch):
    ...
    monkeypatch.setattr(
        "src.setup.app_prompts.ask_text", lambda prompt, default=None: seq.pop(0)
    )
